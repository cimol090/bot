     ...
     # clear
      ...
      if cmd == "whois" or cmd == "who" or cmd == "w":
          ...
          if sid_ver == None:
            ...
            return
          room.message("Currently known alias(es) of <b>%s</b>: %s"% (args.title(), sid_ver), True)

      if room.getLevel(self.user) > 0:
          if user.name not in sid_whois:
            sid_var = message.puid
            sid_whois.update({user.name:[sid_var]})
          if user.name in sid_whois:
            sid_var = message.puid
            if sid_var in sid_whois[user.name]:
              pass
            else:
              sid_whois[user.name].append(sid_var)
          f = open('sid_whois.txt', "w")
          f.write(str(sid_whois))
          f.close
      ...
      if cmd == "mute" and self.getAccess(user) >= 3:
        ...
        if not args:
          if room.name not in simlock:
             ...
             f.write("\n".join(simlock))
             f.close()
             room.message("Muted: %s" % room.name,True)
          else:
             room.message(":|")
      ##Simi unlock
